...
func (a *Stargate) Extract(chain string, events model.Events) model.Results {
	...
	for _, outPair := range outPairs {
		...
		res := &model.Result{
			...
			FromChainId: utils.GetChainId(chain),
		}
		from := "0x" + outPair[0].Data[2+64*2+24:2+64*3]
		if !utils.Contains(from, StargateContracts[chain]) {
			res.FromAddress = from
		}
		res.ToChainId, _ = new(big.Int).SetString(outPair[0].Data[2:66], 16)
		res.ToChainId = a.ChangeID(res.ToChainId)
		...
		res.Amount = utils.HexSum(outPair[0].Data[2+64*3:2+64*4], outPair[0].Data[2+64*4:2+64*5],
		...
		res.Amount.Mul(res.Amount, convRate)
		detail := &OutDetail{
	...
	for _, inPair := range inPairs {
		...
		res := &model.Result{
			...
			ToChainId: utils.GetChainId(chain),
		}
		res.FromChainId, _ = new(big.Int).SetString(inPair[0].Topics[1][2:], 16)
		res.FromChainId = a.ChangeID(res.FromChainId)
		...
		res.Amount = utils.HexSum(inPair[1].Data[2+64 : 2+2*64])
...
func (a *Stargate) GetPoolToken(chain, pool string) (string, error) {
	...
	// 0xfc0c546a: token()
	raw, err := p.Call("", pool, "0xfc0c546a", nil, nil)
	if err != nil {
		return "", err
	}
...
func (a *Stargate) GetPoolConvertRate(chain, pool string) (*big.Int, error) {
	...
	raw, err := p.Call("", pool, "0xfeb56b15", nil, nil)
	if err != nil {
		return nil, err
	}
