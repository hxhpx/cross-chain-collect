...
func NewSynapseCollector() *Synapse {
	return &Synapse{}
}
...
func (a *Synapse) Contracts(chain string) []string {
...
func (a *Synapse) Extract(chain string, events model.Events) model.Results {
	...
	for _, e := range events {
		...
		if e.Topics[0] == TokenDeposit || e.Topics[0] == TokenDepositAndSwap ||
			...
			res.ToChainId, _ = new(big.Int).SetString(e.Data[2:2+64], 16)
			res.Amount, _ = new(big.Int).SetString(e.Data[2+128:2+192], 16)
		...
		if e.Topics[0] == TokenMint || e.Topics[0] == TokenMintAndSwap ||
			...
			res.Amount, _ = new(big.Int).SetString(e.Data[2+64:2+128], 16)
