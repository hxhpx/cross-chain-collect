...
const { GETBLOCK_BSC, PRIVATE_KEY, CONTRACT_BSC_FOR_USDT } = process.env

const Web3 = require('web3');
//set wss provider to connet network
const web3 = new Web3(new Web3.providers.WebsocketProvider(GETBLOCK_BSC));
...
async function main() {
  while(fromBlk >= 21000000) {
    contract.getPastEvents('LogAnySwapIn', {
      fromBlock:fromBlk,
      toBlock:toBlk
      ...
      if(error){
        ...
        result.forEach(function(it,idx) {
          if(it.returnValues.fromChainID == 1 && it.returnValues.from != CONTRACT_BSC_FOR_USDT){
            var txn = it.transactionHash;
            var account = it.returnValues.to;
            var txhash = it.returnValues.txhash;
            //var fromChainID = it.returnValues.fromChainID;
            //var toChainID = it.returnValues.toChainID;
            var amount = it.returnValues.amount;
            var obj = txn+'\t'+txhash+'\t'+account+'\t'+amount+'\n';
            fs.writeFile('./pastEvent/bsc-from-ETH-USDT.json',obj,{flag:'a'},function(error){
